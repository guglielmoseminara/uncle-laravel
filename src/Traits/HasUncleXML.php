...
trait HasUncleXML {
        ...
        if(array_key_exists($method, $this->xml_relations)){
            ...
            switch($relation['type']){
                case 'hasOne':
                    ...
                    return $this->uncleHasOne($relation['resource'], $relation['model'], $foreignKey, $localKey);
                    break;

                case 'hasMany':
                    $foreignKey = (isset($relation['foreignKey']))? $relation['foreignKey'] : null;
                    $localKey   = (isset($relation['localKey']))? $relation['localKey'] : null;
                    return $this->uncleHasMany($relation['resource'], $relation['model'], $foreignKey, $localKey);
                    break;

                case 'belongsTo':
                    $foreignKey  = (isset($relation['foreignKey']))? $relation['foreignKey'] : null;
                    $ownerKey    = (isset($relation['ownerKey']))? $relation['ownerKey'] : null;
                    return $this->uncleBelongsTo($relation['resource'], $relation['model'], $foreignKey, $ownerKey);
                    break;
            }
    ...
    private function uncleHasOne($resource, $model, $foreignKey = null, $localKey = null){
        return $this->hasOne(App::make($resource)->getModelClassPath($model), $foreignKey, $localKey);
    }

    private function uncleHasMany($resource, $model, $foreignKey = null, $localKey = null){
        return $this->hasMany(App::make($resource)->getModelClassPath($model), $foreignKey, $localKey);
    }

    private function uncleBelongsTo($resource, $model, $foreignKey = null, $ownerKey = null ) {
        return $this->belongsTo(App::make($resource)->getModelClassPath($model), $foreignKey, $ownerKey);
    }
